...
3. **국소성(Locality)**: 문장을 한꺼번에 전부 재작성하지 않는다. AI 티가 있는 구간만 수술적으로 고친다.
   - **`register: course` carve-out**: 교육자료 산문은 국소 span 편집만으로는 문단 흐름이 뚝뚝 끊긴다. course register에서는 **문단 단위 재구조화를 허용**한다(문장 순서 조정·연결·분할·병합). 단 **문서 전체 재작성은 여전히 금지**하고, 문단의 사실·주장·수치·순서는 보존한다. §1.Y "문단 흐름 복원" 레시피 참조.
4. **자연성 우선, 완벽성 차순(Natural > Perfect)**: 과하게 문학적으로 고치지 않는다. 일상 한국어 필자의 중간값 리듬을 목표.
   - **흔한 어휘 우선(plain-language)**: 번역투를 걷어내려다 잘 안 쓰는 문어체·문학체 어휘("포개다·결이 다르다·복리로·파고들다·통로")로 갈아타는 것은 **과교정**이다. 치환어는 "일반 기술 문서·블로그에서 쓰는 말인가?"를 기준으로 고른다. drift→평이 대응표는 `quick-rules.md` "흔한 어휘 우선" 절 참조. 반대로 **구어 은유·영어 관용구 직역**(`칸을 먹다`, `걸려 넘어지는 지점`, `되는 길`)으로 가는 것도 같은 기준의 위반이다. 원문에 그런 어구가 있으면 평이한 서술어로 바꾼다(2026-09-22). 같은 절의 둘째 대응표 참조.
...
- **가운뎃점(·)**: 세 항 이상 나열은 쉼표로, 두 항 대비는 "와/과"로. 표 셀·코드블록 안의 압축 표기만 남긴다.

#### J-말미. register: course의 부호 금지선 (강화)

`register: course`에서는 **산문의 줄표(—)와 가운뎃점(·)이 0회**다(근거: 저장소 `docs/course-plan.md` §3.10). 1~2회 허용도 없다.

- 윤문이 문장을 재구성하면서 줄표를 **새로 만들어 넣지 않는다.** 부연을 붙일 자리가 없으면 문장을 나눈다.
- 줄표를 **콜론으로 치환하지 않는다.** C-10(`X: Y` 부제 공식)으로 옮겨갈 뿐이다. 헤딩은 한 구절로 합친다.
- 예외는 표 셀, 코드펜스, frontmatter, 수식, 파일 경로, 외부 문헌 제목의 축자 인용뿐이다.
- `naturalness-reviewer`는 이 두 부호의 산문 잔존 수를 course register 판정 항목에 포함한다. 검산은 `bash scripts/lint-lesson.sh <파일>`의 G절.

### 1.Y. 문단 흐름 복원 (Paragraph-flow · register: course 전용)

국소 span 편집이 끝난 뒤, course register에서만 문단을 한 덩어리로 다시 읽고 흐름을 복원한다. 목표는 "문장은 깨끗한데 문단이 어색한" 상태 해소다. **사실·주장·수치·인용·정보 순서는 보존**하고 연결·리듬만 손본다.
...
- **교육자료 추가 금지선**: 코드펜스·표·LaTeX·Mermaid 블록은 **경계도 순서도 건드리지 않는다.** 이들을 설명하는 앞뒤 산문 문단만 대상이다. 블록을 사이에 둔 두 문단을 병합하지 않는다.

> 이 레시피는 `korean-style-rewriter`가 register=course일 때 span 윤문 직후 1패스로 수행하고, `naturalness-reviewer`가 "문단 전체 흐름·연결" 항목으로 검수한다.
...
### `register: course` 밴드 (교육자료)

전체 문자 변경률을 주지표로 쓰지 않는다. lesson.md는 문자의 40% 이상이 코드·표·수식(전량 보존 대상)이라 **전체 문자율이 구조적으로 낮게 나오고**, 그 수치로 재윤문을 지시하면 오히려 과윤문을 유발한다.

| 대상 | 주지표 | 목표 |
|---|---|---|
| **`course/**/lesson.md` 전부** (티어와 무관) | 산문 라인 변경률 + 잔존 burden 감소율 | 라인 **20~50%** · burden **80%↓** 이상 |
| lesson이 아닌 짧은 산문 문서 | 전체 문자 변경률 | **5~30%** |

- **산문 = 코드펜스·표 행·LaTeX 블록·Mermaid·frontmatter를 제외한 라인.** 전체 문자율은 참고 기록으로만 남긴다.
- 주지표 하한 미달 + S1 잔존 → **저윤문**, 재윤문. 상한 초과 → **과윤문**, 롤백.
- 상한을 올려도 안전한 이유는 `content-fidelity-auditor`가 의미 불변을 독립 감사하기 때문이다.
- SSOT는 `docs/course-plan.md` **§1의 「변경률 판정 지표」 박스**다. 위 표는 사본이며 불일치 시 course-plan이 우선한다. **2026-09-01에 티어로 가르던 규정을 폐기했다** — lesson은 티어와 보호 구간 비율과 무관하게 산문 라인으로 판정한다.
