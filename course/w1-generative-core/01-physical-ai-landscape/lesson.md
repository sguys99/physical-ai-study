...
| §3 | 스택 5계층 | 명령 하나가 위에서 아래로 내려가는 경로 추적 | 12분 |
...
Physical AI 스택은 **계층적 제어**(supervisory / cascade control)의 재발견입니다. 캐스케이드 제어에서 바깥 루프는 목표를 정해 안쪽에 setpoint로 넘깁니다. 바깥은 느리게 판단하며 안쪽은 빠르게 추종합니다.

로봇 스택도 똑같습니다. 상위 지능이 플래너, 전신 제어가 서보 루프, 그 사이가 setpoint 인터페이스입니다. **달라진 것은 바깥 루프뿐**입니다. 미분방정식 자리에 30억 파라미터 트랜스포머가 들어앉았습니다.

모델 기반 노선은 동역학 식에 MPC를 얹습니다. 그 한계는 늘 접촉, 마찰, 변형체, 시각처럼 **모델링이 안 되는 영역**이었습니다. Physical AI는 그 영역만 학습으로 대체하고 고주파 서보 루프는 그대로 둡니다. 남는 것은 end-to-end가 아니라 하이브리드 계층 구조이고 이 모듈이 그 설계도입니다.
...
**BPE**부터 짚습니다. 문자 단위로 시작해 자주 붙어 나오는 문자 쌍을 정해진 횟수만큼 병합하면 그 조각 목록이 어휘 사전이 되고 모든 문장은 그 사전의 인덱스 열이 됩니다. 길이가 제각각인 연속 원신호가 **유한한 정수 목록**으로 바뀝니다. 이것이 요점입니다. 그래야 **자기회귀** 모델이 다음 인덱스의 확률을 예측합니다.

FSQ를 "액션의 BPE"라 부르는 비유가 문자 그대로 통하는 이유입니다. 연속 관절 신호를 유한한 **코드북** 인덱스로 바꿔 상위 모델이 다룰 수 있게 만드는 것이 같습니다. W1-M5에서 직접 구현합니다.

### 1.3 이 모듈은 좌표계다

여기서 세우는 것은 5계층과 주파수, 지연 예산 부등식, 승부처 L3, 데이터 피라미드, 회사 스택 배치도입니다. 회수 지점은 [deep-dive.md](deep-dive.md) §8.2에 표로 있습니다. 마지막 항목이 중요합니다. 배치도는 **가설**이고 캡스톤 검증까지가 4주 과정입니다.
...
**Embodied AI**와 거의 같은 뜻인데 이 문서는 스택 전체를 가리키는 쪽으로 씁니다.
...
**각 경계에서 자료형이 어떻게 바뀌는지**가 요점입니다. 사람이 말합니다. "빨간 컵 있는 곳으로 가."

**L5 인지와 매핑 (0.5~5 Hz)** 는 언어를 좌표로 바꿉니다. 시맨틱 맵에서 "빨간 컵"을 질의해 `[2.3, 1.1, 0.8]` 같은 좌표를 꺼냅니다. 본 적 없는 표현도 처리하므로 **open-vocabulary**여야 합니다. DualMap 담당입니다.
...
**L2 전신 제어 WBC (50~500 Hz)** 는 신경망 forward 한 번이 전부이고 그 한 번에 균형, 접촉, 모션 트래킹이 걸립니다. **로봇이 넘어지느냐가 여기서 갈립니다.** L2가 L4보다 최대 500배 빠릅니다. 그 간극은 §4에서 메웁니다.
...
- **제어 대응**: 상태 피드백. **맨 아래가 여전히 고전 제어**입니다. 상위가 트랜스포머여도 모터를 돌리는 것은 이 비례-미분 항입니다
...
§3.1의 경로를 한 장으로 정리합니다. 아래로 갈수록 **빨라지고 멍청해집니다.**
...
오른쪽 열이 전부 "팀 확인 필요"인 데서 출발합니다. 이 표를 채우는 것이 **온보딩의 목표**입니다.

주파수 중에 하나는 물리가 정합니다. L2를 50 Hz 아래로 내리면 로봇은 넘어집니다. 발이 미끄러지기 시작하면 자세가 무너지기까지 **200 ms** 남짓이라, 균형 회복 반사가 그보다 느리면 손쓸 시간이 없습니다. 이 **200 ms 문턱**은 뒤에서 계속 돌아옵니다. §4의 지연 예산이 최소 청크 길이를 정할 때 기준이 됩니다. W2-M1에서는 ACT의 기본값이 이 문턱과 충돌하는지 따질 때 다시 씁니다.
...
- 위쪽 경계는 **기능 분리**입니다. L4와 L5는 둘 다 느립니다. 두 계층을 가른 것은 인지(무엇이 어디 있는가)와 행동 의도(무엇을 할 것인가)의 차이이지 대역폭이 아닙니다
- 아래쪽 두 경계는 대역폭 분리입니다. L1/L2와 L2/L4는 규칙을 넉넉히 만족하며 여기서는 계층 분리를 물리가 뒷받침합니다

계층 분리에는 **두 종류의 논리**가 섞여 있습니다. 계산은 [`practice/01_stack_frequency_budget.py`](practice/01_stack_frequency_budget.py)에서 재현합니다.

### 3.5 승부처는 왜 L3인가

블록도에서 L3 옆에 ★ 승부처 ★를 붙여뒀습니다. L4와 L2를 각각 잘 만드는 것보다 둘을 잇는 **인터페이스가 성능과 교체 가능성을 결정**합니다.
...
$\lceil 50 \times 0.12 \rceil = 6$ 스텝이 버려집니다. **16개를 예측했는데 쓸 수 있는 것은 10개뿐**입니다. 이것이 부등식에 $\tau$가 들어가는 이유입니다. 아래는 파란 막대가 사용, 빨간 막대가 손실입니다.
...
- **그 100만조차 단일 플랫폼**이라 옮겨갈 수 없습니다. 그래서 방법론 대부분이 라벨 없는 상단을 끌어내리는 데 쏟아집니다
...
latent action을 이산화하려면 양자화기가 필요한데 FSQ가 그것입니다. §3.5에서 L3 인터페이스로 본 FSQ가 여기서는 데이터 문제의 도구로 나옵니다. 같은 기술이 두 문제를 건드립니다. 이 분야가 그렇습니다.

> 📌 **여기까지 정리**
> - 위쪽은 넘치는데 **액션 라벨이 없고** 아래쪽은 정확한데 세 자릿수 이상 적다
> - 우회로는 역추정(IDM), 대리 코드(latent action), 환경 모델링(월드모델)이다
...
계보 그림에는 누가 누구를 이었는지만 담겨 있습니다. 쟁점은 이렇습니다.
...
- **DualMap 출력 형태.** 좌표를 넘기는지, 질의 API인지, 언어 그대로인지
...
회사 스택은 L4와 L2를 분리해 L3로 잇습니다. Gemini Robotics 2(2026-07-31)는 반대 방향으로 보입니다(내부 구조 미공개).
...
계층은 취향이 아니라 **주파수와 지연 예산이 강제한 결과**입니다. 30억 파라미터 모델을 1 kHz로 돌릴 수 없고 균형 회복 반사를 5 Hz로 할 수도 없습니다(§4). 상위 모델이 전신 제어를 흡수해도 서보 루프는 따로 돕니다. 달라지는 것은 경계선의 위치뿐입니다.

### 오해 2 "계층 경계는 전부 대역폭 분리로 설명된다"

§3.4의 실측을 보면 그렇지 않습니다. L1/L2는 ×8.9, L2/L4는 ×50이지만 **L4/L5는 대표 ×2.0, 최악 ×0.20으로 역전**합니다. 위쪽을 가른 것은 인지와 행동 의도라는 기능 차이입니다. 두 논리가 섞여 있음을 알아야 새 아키텍처에서 "이 경계는 왜 있는가"를 묻게 됩니다.
...
업계는 그래서 sim2sim 검증(학습 시뮬 → 다른 시뮬 → 실기)을 씁니다. 플랜트 모델 하나로 튜닝한 제어기가 실기에서 발산하는 것과 같습니다. 도메인 랜더마이제이션은 **강인제어의 불확실성 집합**입니다. → W3-M5
...
**신뢰도 각주** (2026-08-01). 여섯 항목(G1 자유도, GR00T N1.6 공개일, Cosmos 3 릴리스, GEAR-SONIC 체크포인트, Figure/Tesla/1X 정량치, §3.3 오른쪽 열과 §7.1 배치도)은 수치를 비웠거나 교차확인으로만 채웠습니다. 전문은 **[deep-dive.md](deep-dive.md) §6**. **빈칸이 틀린 숫자보다 낫습니다.**
