...
[W1-M1 §4.4](../../w1-generative-core/01-physical-ai-landscape/lesson.md)는 액션을 묶는 것을 **지연 흡수 장치**로 소개하고 "W2-M1에서 실제 하이퍼파라미터로 다시 만난다"는 약속으로 끝납니다. 그 약속을 갚으면서 절반을 정정하게 됩니다. **ACT의 원 논거는 지연이 아니라 오차 누적을 줄이는 것**이고 지연 흡수는 부수 효과입니다. 그 차이는 §3.6에서 숫자로 드러납니다.
...
- **최악 반응 지연이 3.3초입니다.** [W1-M1 §3.3](../../w1-generative-core/01-physical-ai-landscape/lesson.md)의 200 ms 문턱의 **16배 밖**입니다. 이 대비가 이 모듈에서 가장 중요한 숫자입니다.
...
- **청크에서 낙상 모드가 제거됩니다.** 청크로 다뤄지는 채널은 상체 조작이고 200 ms 문턱이 걸린 균형은 별도 루프에 있어서 §3.6의 결론이 청크 채널에 걸리지 않습니다. [W1-M1 §3.4](../../w1-generative-core/01-physical-ai-landscape/lesson.md)의 대역폭 분리(안쪽 루프를 바깥보다 한 자릿수 이상 빠르게 돌리는 그 관행)를 수집 하드웨어 수준에서 구현한 셈입니다.
