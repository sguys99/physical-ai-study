# CLAUDE.md — Physical AI 온보딩 스터디

## 이 저장소

Physical AI 온보딩 4주 스터디의 학습자료 저작 + 실습 저장소입니다.
`docs/physical-ai-4week-master-plan.md`(이하 **마스터 플랜**)가 **SSOT**이며, 여기서 하는 모든 작업은 그 플랜의 모듈(`W1-M1` 형식)을 토픽으로 구현하는 것입니다.

**세션 시작 시 반드시**: 작업 대상 모듈이 정해지면 마스터 플랜에서 **해당 모듈 섹션 + §2(회사 스택) + §3(갭 분석)을 먼저 읽고** 시작하세요. 플랜 없이 일반 지식으로 쓰면 회사 스택 연결이 빠져 가치가 절반이 됩니다.

---

## 저장소 구조

```
docs/
  physical-ai-4week-master-plan.md   # SSOT — 커리큘럼 전체 설계 (수정 시 사용자 승인 필요)
  course-plan.md                     # 집필 규약 SSOT — 작성 표준·규약 (§1~§4, §6~§8)
  course-status.md                   # 집필 현황 — §0 대시보드 + §5 모듈별 체크박스
  course-plan-log.md                 # 진행 메모 §9 — 규약이 왜 그렇게 정해졌는지의 기록
  progress.md                        # 학습 실행 로그 — 무엇을 돌렸나·막힌 지점·소요시간·GPU 비용
course/
  w1-generative-core/                # 주차 = Phase
    01-physical-ai-landscape/        # 토픽 (= 모듈 W1-M1)
      ├─ lesson.md                   # 이론 + 코드 실습 본문
      ├─ practice/                   # 실행 가능한 .py / notebook.ipynb / requirements.txt
      └─ labs/                       # 단계별 핸즈온 명령 + 예상 출력
    02-simulator-bootcamp/           # W1-M2
    ...
  w2-policy-vla/  w3-wbc-rl/  w4-worldmodel-nav/
  capstone/                          # W4-M5 산출물 (스택 해설 문서 + 발표자료)
notes/
  glossary.md                        # 용어집 (매일 5개 누적)
  papers/                            # 논문 노트 1편 = 1파일
  repo-tours/                        # 외부 리포 투어 노트 1개 = 1파일
  questions-for-team.md              # 팀 확인 질문 누적 ← 온보딩의 핵심 산출물
repos/                               # 외부 리포 클론 (OpenHomie, FAST_LIO, HoloAgent, DualMap 등) — gitignore
artifacts/                           # 롤아웃 mp4, 학습 커브, 다이어그램 (대용량은 gitignore)
img/
CLAUDE.md   README.md   .env.example   .gitignore
```

### 명명 규칙

- 주차 폴더: `w{N}-{slug}` / 토픽 폴더: `{NN}-{topic-slug}` — **숫자 prefix가 정렬 순서를 결정**
- 토픽 번호는 모듈 번호와 1:1 (`W2-M3` → `course/w2-policy-vla/03-vla-lineage/`)
- `lesson.md` 상단에 모듈 ID를 프론트매터로 명시: `module: W2-M3`
- 링크는 상대 경로, 이미지는 `../../../img/` 사용

---

## 사용자 프로필

- 데이터 사이언티스트 / ML 엔지니어 / 에이전트 엔지니어. Control engineering 경력도 있음
- 영어 논문 독해 가능. 문서·설명은 **한국어**

### 교육자료의 학습자 전제 (2026-08-25 개정, 근거는 [course-plan-log §9.19](docs/course-plan-log.md))

> ⚠️ **경력을 근거로 설명을 생략하지 마세요.** 이전 규약은 제어공학 배경을 설명의 앵커로 삼았고, 학습자가 두 차례에 걸쳐 "빗대어 설명하는데 이해가 안 된다, 너무 어렵다"고 지적했습니다. **주니어에게 처음부터 설명한다는 전제로 씁니다.**

**안다고 가정하는 것은 셋뿐입니다.**

- ML/DL 기본 (학습, 손실, 신경망, 과적합)
- LLM 기본 (프롬프트, 토큰이라는 말, 파인튜닝)
- Agent와 RAG 응용 실무

**그 밖은 전부 모른다고 가정합니다.** 제어공학(MPC, 상태공간, 캐스케이드 제어, PD 서보, 대역폭), 생성모델 내부 기제(BPE, autoregressive, codebook, ELBO, KV 캐시), 로봇 전 영역(시뮬레이터, VLA, WBC, SLAM, 미들웨어), 그리고 그 모듈이 다루는 개념 자체.

**설명 방식** (상세 규약은 [course-plan §3.9](docs/course-plan.md))

- **비유는 보조입니다.** 본문은 비유 없이 완결되어야 하고, 비유는 본문 뒤에 `<details>` 박스로 답니다. 박스 안에서도 앵커 개념을 한 문단으로 재설명합니다. 용어표에 `제어·LLM 비유` 열을 두지 않습니다.
- **새 용어는 첫 등장 자리에서 두세 문장으로 풀어 씁니다.** 정의보다 먼저 쓰지 않습니다. 한 줄 정의는 절 끝 되짚기 표에만.
- **모든 영역을 기초부터.** "제어 배경이면 설명이 필요 없을 겁니다", "당신은 이미 알고 있다" 류의 설명 회피 문장 금지.
- **수식에는 숫자를 대입한 손계산이 붙습니다.** 결과만 제시하고 계산을 실습 코드로 외주하지 않습니다. 모든 기호는 그 자리에서 정의.
- **다이어그램과 그림을 적극 활용하고 「읽는 법」 캡션을 답니다.** 개념 그림은 SVG를 직접 작성해 `img/`에 둡니다 (관례는 [course-plan §3.2](docs/course-plan.md)).
- **서술어는 기술 문서의 평이한 말로.** 사물 은유(칸을 먹다, 영상을 뽑다)와 영어 관용구 직역(걸려 넘어지는 지점, 되는 길)을 쓰지 않습니다. 쉬운 말은 구어가 아니라 기술 문서에서 그대로 쓰는 말이고, 그림 라벨과 캡션도 같습니다 (규약은 [course-plan §3.11](docs/course-plan.md), 근거는 course-plan-log §9.22).

---

## 회사 기술 스택 (모든 lesson이 여기로 연결되어야 함)

| 계층 | 기술 | 역할 |
|---|---|---|
| L5 인지 (기하) | **FAST-LIO2** ★ | LiDAR-관성 오도메트리. 3D 점군 맵(pcd) + 현 위치 추정 + 전처리 LiDAR |
| L5 인지 (시맨틱) | **FSR-VLN** ⚙ / **DualMap** 🔍 | open-vocabulary 시맨틱 매핑. 자연어 질의 → 목표 좌표 |
| L4 상위 지능 | **GR00T N1.x VLA** ★ / World Model | 목표 → 행동 의도 (조작 경로) |
| L4→L2 내비 경로 | **Nav2** ★ | 2D 격자 맵 → Global Path 2 Hz / Local Path 20 Hz / 속도 명령 → WBC |
| L3 액션 인터페이스 | **FSQ 기반 계층 모델** ★ | 상위 모델 ↔ 하위 제어기를 잇는 이산 액션 토큰 (조작 경로) |
| L2 전신 제어 | **GEAR-SONIC** ★ / **HOMIE** ★ | WBC 모션 트래킹 파운데이션 정책 / 텔레옵 기반 데이터 수집 |
| L1 하드웨어 | **Unitree G1** ★ | 23~43 DoF 휴머노이드, DDS/SDK. Livox Mid-360 LiDAR + RGB-D |

**상태 표기**: ★ = 현재 운영 중, ⚙ = 구현 중, 🔍 = 검토 중 (2026-08-29 사내 「GenP Navigation 모듈 진행 현황」 기준)

**L2로 내려가는 길이 둘입니다.** 내비게이션은 Nav2가 속도 명령을 WBC로 직접 보내고 L4 VLA를 거치지 않습니다. 조작은 VLA → FSQ 토큰으로 갑니다. 자세한 것은 [W1-M1 §7.4](course/w1-generative-core/01-physical-ai-landscape/lesson.md).

3-pass 정독 대상 6편: FSQ(2309.15505) / SONIC(2511.07820) / HOMIE(2502.13013) / pi0(2410.24164) / Diffusion Policy(2303.04137) / DualMap.

---

## 실행 환경

**로컬 RTX 3080 12GB (WSL2 CUDA 가능) 주력 + 필요시 클라우드 GPU. G1 실기 접근 불가. 시뮬레이터 경험 전무.**

W1~W2는 로컬로 완결됩니다. 클라우드는 W3 대규모 병렬 학습이 로컬에서 하룻밤을 넘길 때만 씁니다. 상세는 마스터플랜 부록 A.0.

| 역할 | 기본 선택 | 대안 |
|---|---|---|
| 물리 시뮬 | MuJoCo (pip) | Genesis |
| RL 학습 | mujoco_playground (MJX, GPU 병렬) | — |
| sim2sim 검증 | unitree_mujoco (표준 MuJoCo) | — |
| 모방학습·VLA | LeRobot | — |
| 로봇 모델 | mujoco_menagerie (G1) | unitree_ros |
| 실험 추적 | W&B 또는 TensorBoard | — |
| Isaac Sim/Lab | **[P1] 개념만, 실행은 선택** | — |

- 렌더링은 항상 headless: `MUJOCO_GL=egl`. **뷰어를 띄우는 코드를 제안하지 말 것** — 결과는 mp4/png로 저장해 확인.
- 장시간 작업은 tmux. 체크포인트·데이터셋은 퍼시스턴트 볼륨에 (인스턴스는 소모품).
- **비용 주의**: 30분 이상 걸릴 학습 전에 예상 소요 시간·GPU 시간을 먼저 알리고 확인받을 것. 종료 후 인스턴스 정지 리마인드.
- 패키지 설치·환경 변경은 실행 전에 무엇을 왜 설치하는지 한 줄로 알릴 것.
- 버전이 빠른 항목(LeRobot, playground)은 작성 시점에 공식 문서로 재확인.

---

## 작성 표준

> 📍 **집필 규약의 SSOT는 [`docs/course-plan.md`](docs/course-plan.md)입니다.** 아래는 집필 자세와 금지 사항의 요약이고, 충돌하면 course-plan이 이깁니다.
> **수치와 판정 기준은 여기에 적지 않습니다.** 분량 밴드, 변경률 밴드, 검사 문턱 같은 값을 복사해두면 반드시 어긋납니다. 실제로 분량 규정이 3중 불일치 상태였습니다([course-plan-log §9.11](docs/course-plan-log.md)). 값이 필요하면 `docs/course-plan.md`와 `scripts/lint-lesson.sh`를 엽니다.

### `lesson.md` — 이론 + 코드 실습 본문

| 항목 | 규약 위치 |
|---|---|
| **3층 문서** — `eli5.md` / `lesson.md` / `deep-dive.md` | course-plan **§2.1** |
| **섹션 순서(고정)** · 절별 4부 구조 · 도입 산문 필수 | course-plan **§3.7** |
| **용어 도입** — 첫 등장 인라인 정의 · 절 끝 되짚기 표 · 역참조 금지 | course-plan **§3.8** |
| **학습자 전제와 비유 강등** | course-plan **§3.9** |
| **문장부호** — 줄표(—)·가운뎃점(·) 금지 | course-plan **§3.10** |
| **워크드 예제 · 헷갈림 박스 · 퀴즈 3단 · 문체 통일 · 평이한 서술어** | course-plan **§3.11** |
| 시각자료 · 그림 캡션 · SVG · 표를 쓰지 않을 조건 | course-plan **§3.2** |
| frontmatter 스키마 | course-plan **§3.1** |
| 분량(산문 문자) · `est_reading_min` · `deep-dive.md` 이관 | course-plan **§4** |

핵심만 다시:

- **한 모듈은 세 층입니다.** `eli5.md`(그림 위주 입문, 수식 0)를 먼저 읽고, `lesson.md`가 본문이고, `deep-dive.md`가 심화입니다.
- 구성은 **이 모듈 지도(목차) → 배경 → 단계별 상세 → 회사 스택 연결 → 한 장 정리** 순서. **대절 제목 다음 첫 요소는 반드시 도입 산문 3~5문장**이고 표나 그림이 아닙니다. 절 안에서는 **개념 → 그림 → 예시와 숫자 → 수식과 손계산 → 비유 박스**.
- **처음 쓰는 용어는 그 자리에서 두세 문장으로 풀어 씁니다.** 약어는 최초 1회 전개하고 첫 세 번은 한국어를 병기합니다. **정의보다 먼저 쓰지 않습니다.**
- **표는 비교의 도구이지 서술의 그릇이 아닙니다.** 축이 2개 이상인 대조만 표로. 나열과 순차와 근거는 불릿, 논증은 산문.
- **산문에 줄표(—)와 가운뎃점(·)을 쓰지 않습니다.** 부연은 쉼표나 괄호로, 나열은 쉼표로, 헤딩 부제는 한 구절로 합칩니다. 콜론 부제(`X: Y`)도 대안이 아닙니다. 표 셀과 코드블록 안은 예외.
- 최신 정보는 웹 검색으로 보강하고 출처 URL을 남길 것.
- **모르는 것은 모른다고 쓸 것.** 특히 회사 내부 구현(FSQ 모델의 실제 구조 등)은 추측하지 말고 `notes/questions-for-team.md`에 질문으로 적립.
- 집필 후 **`bash scripts/lint-lesson.sh <파일>`로 규약 준수를 확인**합니다.

### `practice/` · `labs/`

규약은 course-plan **§3.4**(practice 코드·jupytext)와 **§2**(산출물별 내용), **§3.5**(미검증 배지)를 따릅니다. 원칙만:

- **스모크 경로 우선** — 무거운 학습은 `--smoke`로 먼저 완주되게. GPU 시간 낭비와 입문자 좌절을 동시에 줄이는 장치.
- 코드는 학습용이므로 **읽기 쉬운 것 > 최적화**. 핵심 수식이 코드 어디에 대응하는지 주석으로.
- `labs/`는 각 단계마다 **명령 + 예상 출력(성공 판정 기준)**. 시뮬 입문자에게 가장 중요한 부분.
- 작성 후 가능한 범위에서 직접 실행해 검증. 실행 불가한 부분은 그렇다고 명시.

### `notes/papers/` — 논문 노트

마스터 플랜 §11.2 템플릿. 1편 = ①한 줄 요약 ②아키텍처 그림 ③우리 스택과의 연결 ④의문점. ④는 `questions-for-team.md`에도 복사. 3-pass는 위 6편만, 나머지는 2-pass까지.

### `repos/` — 리포 투어

마스터 플랜 §11.3 템플릿. 디렉토리 맵 → 진입점부터 콜스택 순 핵심 파일 5개 → 논문 수식↔코드 매핑 → 주요 하이퍼파라미터 → 최소 실행 가이드 → 우리 스택 통합 시 손댈 인터페이스. **파일 경로:라인을 인용할 것.**

---

## 전역 규칙

- **집필 요청은 선행 탐색 스킬을 거치지 않습니다.** 요구사항은 마스터플랜과 `docs/course-plan.md`에 이미 확정돼 있어서 `brainstorming`이나 `using-superpowers` 류의 범용 선행 스킬이 여는 요구사항 탐색 대화는 여기서 얻을 것이 없습니다. 모듈 집필과 보강과 윤문은 `/pai-course-author`로, 윤문 단독은 `/humanize-korean`으로 직행하세요. 그런 스킬은 새 도구나 스크립트를 설계할 때만 의미가 있습니다.
- **awesome 리스트 통독 금지.** 검색이 필요할 때만 색인으로 사용. 마스터 플랜의 핵심 논문 목록을 벗어나 자료를 무한 확장하지 말 것.
- 서베이 4편(2605.00080, 2512.11362, 2510.16732, 2503.21765)은 **지도이지 목적지가 아님**. 필요한 섹션만 참조.
- 시간 부족 시 컷 순서는 마스터 플랜 §12. **끝까지 지킬 것: FSQ 구현(W1-M5), LeRobot 완주(W2-M2), G1 보행 학습+sim2sim(W3), 캡스톤 문서(W4-M5).**
- 마스터 플랜은 SSOT입니다. 수정이 필요해 보이면 먼저 제안하고 승인받을 것.
- 용어는 마스터 플랜 §3 용어 사전과 일치시킬 것. 새 용어는 `notes/glossary.md`에 추가.
- 토픽 하나를 끝내면 `docs/course-status.md`의 해당 체크박스를 갱신할 것. **집필 절차·규약은 전부 `docs/course-plan.md`가 SSOT다** (문서 구조 §3.7 · 용어 도입 §3.8 · 이해 사다리 §3.9 · 시각자료 §3.2 · frontmatter §3.1 · 분량 §4 · jupytext §3.4 · 미검증 배지 §3.5). 집필은 `/pai-course-author <모듈ID>`로 호출하고, 끝나면 `bash scripts/lint-lesson.sh <파일>`로 검사.
- 실습을 돌린 날은 `docs/progress.md`에 실행 로그(돌린 것·막힌 지점·소요시간·GPU 비용)를 남길 것. 여기 쌓인 실측값이 다음 주 자료의 입력이 된다.
- 원칙: **환경이 안 돌아가는 상태로 다음 주로 넘어가지 않는다.** 셋업이 막히면 이론 진도보다 셋업 해결이 우선.

## [P2] 여유가 생기면

- `site/` — `course/`를 스캔해 정적 사이트로 빌드 + GitHub Pages 배포. W4 캡스톤에서 팀 공유용으로 검토 (kb-study 저장소의 `site/build.mjs` 패턴 재사용 가능).