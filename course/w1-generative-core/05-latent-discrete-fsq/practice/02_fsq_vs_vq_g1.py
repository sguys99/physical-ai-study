...
#
# **이 모듈의 핵심 실습입니다.** lesson §12의 문장 — *"이 모듈의 진짜 산출물은 노트북 하나가 아니라
...
# | 재구성 MSE | 정규화 공간 + **라디안·도 환산** | 사용률만 높고 복원이 나쁘면 의미 없다 (lesson §8 두 번째 오해) |
...
#   (lesson §8 두 번째 오해: 사용률은 필요조건이지 충분조건이 아님).
...
def main(argv: list[str] | None = None) -> None:
    ...
    print("  · 사용률만 보고 결론내지 마세요 — val MSE를 같이 봐야 합니다 (lesson §8 두 번째 오해).")
