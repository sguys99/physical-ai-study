...
lint_one() {
  ...
  # 초독 시간 (2026-08-25 신설, §4. 2026-09-03 개정)
  # `est_reading_min`은 이제 **초독 시간**입니다. 산문에 표와 그림과 수식과 퀴즈 풀이를
  # 더한 값이고, 회귀식 값이 아닙니다. F군이 찍는 회귀식 값은 밴드 비교용 지표일 뿐입니다.
  # 검사는 넷입니다. 상한(FAIL/WARN), 추정값과의 편차(FAIL), §0 「소요」와의 정합(FAIL),
  # 목차 합계(WARN).
  #
  # 2026-09-03: **분량 상한이 여기로 왔습니다.** §4가 산문 문자 상한을 폐지하고
  # 상한을 초독 시간으로 옮겼습니다(180분 초과 FAIL · 150분 초과 WARN). 근거는
  # 마스터플랜 §2의 주 30시간 예산이고, 두 숫자는 실측 다섯의 최댓값과 중앙값입니다.
  # 그런데 `est_reading_min`은 **집필자 자기 신고값**이라 그대로 상한에 쓰면 순환입니다.
  # 그래서 같은 개정에서 **추정값과의 30% 대조를 WARN에서 FAIL로 승격**했습니다.
  # 신고값이 lint 추정치에 묶여야 상한이 실제 문서 크기를 재게 됩니다. 둘은 한 쌍이고
  # 한쪽만 되돌리면 안 됩니다(docs/course-plan.md §4 2026-09-03 박스).
  ...
  if [[ -n "$erm" && "$rd_tot" -gt 0 ]]; then
    ...
    if [[ "$rd_dev" -gt 30 ]]; then
      # 2026-09-03: WARN → FAIL 승격. 아래 상한 검사가 이 신고값을 쓰므로 여기가 느슨하면
      # 상한을 신고로 회피할 수 있습니다(§4 2026-09-03 박스의 「순환을 닫는다」).
      c_fail "est_reading_min ${erm}분이 초독 추정 ${rd_tot}분과 ${rd_dev}% 어긋납니다 — §4: 2026-08-25부터 이 값은 초독 시간(산문+표+그림+수식+퀴즈)입니다. 회귀식 값을 그대로 옮겨 적지 마세요. 2026-09-03부터 이 값이 분량 상한 판정의 입력이라 FAIL입니다"
    else
      c_pass "est_reading_min ${erm}분 ≈ 초독 추정 ${rd_tot}분 (편차 ${rd_dev}%)"
    fi

    # 분량 상한 (2026-09-03 신설, §4). 문자 상한을 대신합니다.
    if [[ "$erm" -gt 180 ]]; then
      c_fail "est_reading_min ${erm}분 — §4 상한 180분 초과. 마스터플랜 §2의 주 30시간 예산에서 W1 이론 몫이 12.5h이고 한 모듈이 3h를 넘으면 주간 예산이 깨집니다. **자르지 말고 deep-dive.md로 이관하세요** — 밴드를 지키려고 설명을 깎는 것이 2차 불만의 원인이었습니다(§9.19)"
    elif [[ "$erm" -gt 150 ]]; then
      c_warn "est_reading_min ${erm}분 — §4 권장 상한 150분 초과(FAIL 문턱은 180분). deep-dive.md로 옮길 것이 있는지 보세요"
    else
      c_pass "est_reading_min ${erm}분 (§4 상한 180분, 권장 150분)"
    fi
  fi
  ...
  #
  # 분모는 **바로 위 도입 산문 검사가 센 `intro_n`을 그대로 씁니다**(2026-08-27 정합).
  # 두 검사의 대상 집합이 「번호 붙은 본문 대절」로 같은데 세는 방법이 달랐습니다.
  # 여기만 `grep -cE '^## [0-9]+\.'`라서 코드펜스 안의 `## 1. 무엇` 같은 줄을 걸러내지
  # 못했고, `h2_blocks()`를 쓰는 도입 산문 검사와 분모가 갈릴 수 있었습니다.
  # 지금 7개 파일에서는 값이 같아 출력이 달라지지 않습니다.
  # 📌 개수도 같은 이유로 코드펜스 안을 세지 않습니다. E군의 💡 박스 계수와 같은 방식입니다.
  ...
  # 셀 120자 상한
  #
  # ⚠️ **awk를 쓰지 않습니다. 2026-09-01에 이식성 결함이 드러났습니다.**
  #    이 검사는 원래 awk의 length()로 셌는데, **macOS 기본 awk(BWK awk)는 문자가
  #    아니라 바이트를 셉니다.** 한글은 UTF-8에서 3바이트이므로 length("한글 8자")가
  #    24로 나오고, 결과적으로 "120자 상한"이 한글 표에서는 **약 40자 상한**으로
  #    동작했습니다. GNU awk는 문자를 세므로 **같은 파일이 환경마다 다른 판정**을
  #    받았고, 실제로 W1-M1이 이 저장소 기록의 FAIL 0과 달리 macOS에서 FAIL 1을
  #    냈습니다(120자 초과 셀은 Python 기준 0개인데 2개로 보고).
  #    `gsub(/[\200-\277]/,"",cell)`로 UTF-8 후속 바이트를 걷어내는 우회도
  #    시도했으나 BWK awk가 그 8진 이스케이프를 해석하지 않아 실패했습니다.
  #    이 스크립트는 다른 검사 대부분을 이미 python3로 하므로 여기도 옮깁니다.
  local over_cells
  over_cells=$(python3 - "$f" <<'PYCELL'
import io, sys
n = 0
for line in io.open(sys.argv[1], encoding='utf-8'):
    if not line.lstrip().startswith('|'):
        continue
    # 앞뒤 빈 조각을 뺀 가운데 조각만 셀입니다 (awk의 i=2..NF-1과 같은 범위)
...
PYCELL
  ...
  #    집필 도구(SKILL.md, lesson-template.md, authoring-checklist.md, pai-agent.md)는
  #    숫자를 복사하지 않고 여기를 가리킵니다. 숫자를 여러 곳에 두었다가 3중 불일치가
  #    **세 번 재발**했습니다. §9.11이 한 번, §9.19가 한 번 해소했는데 2026-08-30
  #    Tier A 상향이 스크립트에만 반영돼 또 되살아났고, 2026-09-01에 네 문서에서
  #    숫자를 **아예 지웠습니다**(§4 2026-09-01 박스).
  #    밴드를 고칠 일이 생기면 여기와 §4 표를 **같은 커밋에서** 함께 고치세요.
  #
  # ── 이력 ──
  # 2026-08-10  세 밴드를 각 +3,000. 고정 바닥을 6,350자로 추정했으나 실측 셋이
  #             8,348~8,711이었다(§9.15).
  # 2026-08-30  Tier A 상한만 21,000 → 23,000(§9.20).
  # 2026-09-01  2차 규약의 구조물 산문화를 반영해 A 21,000~32,000 · B 18,000~26,000으로
  #             잠정 상향하고 Tier C를 폐지. 초과는 FAIL이 아니라 WARN으로 낮추고
  #             판정 신호를 `eli5.md` 존재로 삼았다.
  # 2026-09-03  **문자 상한을 폐지한다.** 산문 밴드는 하한만 남고(A ≥ 21,000 ·
  #             B ≥ 18,000, 미달 WARN) 상한은 A군의 `est_reading_min`으로 옮겼다
  #             (180분 초과 FAIL · 150분 초과 WARN). 근거는 §4의 2026-09-03 박스와
  #             docs/course-author-update-plan.md 8-7a·8-7b의 표본 5개 실측이다.
  #
  # ⚠️ **왜 상한을 지웠는가.** 실측 다섯이 말한 것은 밴드가 모듈의 크기가 아니라
  #    **집필자의 라우팅 결정**을 재고 있었다는 것입니다. 3층 합계(eli5 + lesson +
  #    deep-dive)의 산포는 lesson 단독의 2.6분의 1이고(CV 0.096 대 0.248), 밴드 안에
  #    남은 두 모듈이 곧 `deep-dive.md`를 쓴 두 모듈입니다(Pearson −0.794).
  #    문자 상한은 그 라우팅 결정에 대한 벌점이고, 그 벌점은 §9.19가 진단한
  #    「설명을 깎는 압력」과 같은 방향으로 작동합니다. 밴드 값을 또 올리는 것은
  #    네 번째 「넘겼으니 올린다」라서, 값이 아니라 **밴드가 무엇에 묶여 있는가**를
  #    바꿨습니다. 초독 시간에는 유일하게 밖에서 온 근거가 있습니다(마스터플랜 §2의
  #    주 30시간 예산, W1 이론 12.5h가 그 85~98%).
  case "$tier" in
    A) lo=21000 ;;
    B) lo=18000 ;;
    # ⚠️ Tier C는 2026-09-01에 폐지됐습니다(§4). W1-M3·W1-M4·W4-M1이 B로 갔습니다.
    #    §4의 C 근거 두 개가 「복습이다」와 「제어 배경이면 즉시 이해된다」인데
    #    §3.9(2026-08-25)가 이해 사다리를 폐기해 둘 다 무효가 됐습니다.
    #    아직 frontmatter를 안 고친 파일이 조용히 통과하면 안 되므로 분기는 남기되
    #    B와 같은 하한을 쓰고 아래에서 WARN을 냅니다.
    C) lo=18000 ;;
    *) lo=0 ;;
  esac
  # 아래 메시지가 「Tier C 하한(18000)」처럼 읽히면 안 되므로 표시용 티어를 따로 둡니다.
  local band_tier="$tier"
  if [[ "$tier" == "C" ]]; then
    band_tier="B"
    c_warn "Tier C는 2026-09-01에 폐지됐습니다. frontmatter를 tier: B로 고치세요 — §4: C의 근거 「복습이다」와 「제어 배경이면 즉시 이해된다」를 §3.9(2026-08-25)가 무효화했습니다. 아래 분량 판정은 B 하한으로 냈습니다"
  fi
  # 회귀식 값은 **밴드 비교용 지표**이지 학습자가 볼 시간이 아닙니다. 2026-08-25에
  # `est_reading_min`이 초독 시간으로 재정의됐고(§4), 이 식은 **산문만** 세므로 표와
  # 그림과 수식과 퀴즈 풀이가 빠져 있습니다. 초독 시간은 A군이 따로 추정해 찍고,
  # 2026-09-03부터 **상한 판정도 A군이 냅니다.**
  ...
  if [[ "$lo" -eq 0 ]]; then
    ...
    # 2026-09-03: **하한만 판정합니다.** 상한은 A군의 est_reading_min 검사로 갔습니다.
    # 2026-09-01에 두었던 `eli5.md` 존재 기반 WARN 예외 분기는 **제거했습니다.**
    # 그 분기의 존재 이유는 「초과인데 밴드가 잠정이라 FAIL을 낼 수 없다」였는데
    # 상한 자체가 없어졌으므로 초과라는 판정이 성립하지 않습니다. `eli5.md`의 존재는
    # I군이 계속 봅니다.
    if [[ "$prose" -lt "$lo" ]]; then
      c_warn "본문 산문 ${prose}자 — Tier ${band_tier} 하한 ${lo}자 미달, ${reg_note}. 설명이 얇다는 신호입니다"
    else
      c_pass "본문 산문 ${prose}자 (Tier ${band_tier} 하한 ${lo}자 이상), ${reg_note}"
    fi
  fi
  # 2026-09-01: course/**/lesson.md는 **보호 구간 비율과 무관하게** 산문 라인 판정입니다(§1).
  # 35% 문턱은 1차 규약 문서(보호 51~62%)로 교정된 값인데 2차 규약이 그 비율을 구조적으로
  # 끌어내려 W1-M1은 38%로 문턱 위, W1-M2는 32%로 아래가 됐습니다. 같은 규약으로 쓴 두 문서가
  # 판정 지표를 달리 받으면 안 됩니다. 아래 백분율은 참고 정보로만 찍습니다.
  c_info "총 ${total}자 · 구조물 $((total - prose))자 · 보호 구간 ${protect}% (course-plan §1: lesson.md는 비율과 무관하게 윤문 산문 라인 판정)"
  # 2026-09-03 신설: **라우팅 정보줄.** 상한을 지운 대신 「어디에 뒀는가」를 판정 화면에
  # 드러냅니다. 8-7a 실측에서 3층 합계의 산포가 lesson 단독의 2.6분의 1이었고,
  # 밴드 안에 남은 모듈이 곧 deep-dive.md를 쓴 모듈이었습니다. 이 줄은 판정하지 않습니다.
  # eli5.md나 deep-dive.md가 없는 모듈(W2-M1·W2-M2)에서도 죽지 않아야 하므로
  # 존재와 비어 있지 않음을 둘 다 확인하고 없으면 0으로 셉니다.
  local dir3 e3=0 d3=0 e3_note="없음" d3_note="없음" _t3 _p3
  dir3="$(dirname "$f")"
  if [[ -s "$dir3/eli5.md" ]]; then
    read -r e3 _t3 _p3 < <(prose_metrics "$dir3/eli5.md"); e3_note="${e3}자"
  fi
  if [[ -s "$dir3/deep-dive.md" ]]; then
    read -r d3 _t3 _p3 < <(prose_metrics "$dir3/deep-dive.md"); d3_note="${d3}자"
  fi
  c_info "3층 합계 $((e3 + prose + d3))자 (eli5 ${e3_note} + lesson ${prose}자 + deep-dive ${d3_note}) — §4 2026-09-03: 상한이 사라진 자리에 라우팅을 봅니다. 본문이 길면 deep-dive.md로 옮길 것이 있는지 보세요"
