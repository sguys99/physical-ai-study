...
| §3 | 밀어내기 표기와 가우시안 경로의 아핀 구조 | lesson §3.1 · §5.1 |
| §4 | VP와 RF의 변환 대수, 암묵적 가중의 출처 | lesson §5.4 |
...
## 3. 밀어내기 표기와 가우시안 경로의 아핀 구조

### 3.1 흐름이 밀어내는 밀도의 표기

본문 §3.1은 "속도장이 있으면 확률 경로가 정해진다"는 사실만 남기고 표기를 덜어냈습니다. 흐름 $\psi_t$가 초기 밀도 $p_0$을 밀어 옮겨 만든 밀도를 밀어내기(pushforward)라고 부르고 $p_t=[\psi_t]_\#p_0$으로 씁니다. 아래첨자 $\#$이 그 연산 기호입니다. 이 표기로 적으면 연속방정식은 "$[\psi_t]_\#p_0$이 $\partial_tp_t+\nabla\cdot(p_tu_t)=0$을 만족한다"는 한 줄이 됩니다.

### 3.2 $x_t$만으로 다시 쓴 조건부 속도장
...
## 4. VP와 RF의 변환 대수와 암묵적 가중의 출처

본문 §5.4는 "전체 크기를 다시 맞추고 시각을 다시 매기면 서로 옮겨진다"는 결론과 $(\sigma,\alpha)$ 평면의 그림만 남겼습니다. 변환식은 이렇습니다.
...
본문 §5.4는 "$t$ 샘플링 분포가 눈에 보이는 손잡이로 올라온다"는 결론만 남겼습니다. 그 손잡이가 무엇의 자리를 물려받은 것인지가 여기입니다.

확산 모델의 목적함수를 로그가능도의 하한, 곧 ELBO(Evidence Lower BOund, 증거 하한)에서 유도하면 시각마다 다른 가중치 $\lambda_t$가 붙습니다. DDPM은 그 $\lambda_t$를 전부 1로 놓아 버렸고 그것이 $\mathcal{L}_{\text{simple}}$입니다(W1-M3 §4.4). **DDPM이 ELBO의 $\lambda_t$를 버린 자리에 FM은 $t$ 분포를 놓았습니다.** 가중을 손실 계수로 두느냐 표본 추출 확률로 두느냐의 차이이고, 어느 쪽이든 "어느 SNR 구간을 더 열심히 배울 것인가"를 정하는 같은 자유도입니다.
...
경로 계획에 빗대면 이렇습니다. 로봇이 장애물을 피해 처음 짠 계획은 대개 여러 번 꺾여 있습니다. 그 계획을 실행해보고 실제로 출발한 자리와 도착한 자리만 남긴 다음, 그 두 점 사이의 최단 경로를 다시 푸는 작업을 궤적 재계획 또는 경로 평활화라고 부릅니다. 이때 끝점 배정이 바뀌지 않으므로 계획의 "무엇을 하려 했는가"는 보존되고 "어떻게 가는가"만 펴집니다. reflow가 분포를 보존하면서 직선성만 올리는 것과 같은 구조입니다. 본문 §6.3은 이 대응을 여기로 넘기고 절차와 실측만 남겼습니다.
