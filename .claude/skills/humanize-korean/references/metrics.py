...
_HANJA_BLOCK = {
    ...
    "있는화",
    "되는화",  # placeholder — extend as needed
    "맞아",
    "와서",  # not actually -화 but caught for safety
}
...
def ending_comma_rate(text: str) -> float:
    ...
    boundary_endings = re.findall(r"(?:고|며|지만|면서|아서|어서)(?=[\s,\.!?、。]|$)", text)
...
def _resolve_genre_cells(baseline: dict[str, Any], genre: str) -> tuple[dict[str, Any], str | None]:
...
def compute_all(
    ...
    z_scores["hanja_nominalizer_density"] = _z(
        metrics["hanja_nominalizer_density"] * 100,
        6.0,
        12.0,
        percent=False,
    )
...
def _main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--baseline", default=None, help="Override baseline JSON path")
