...
> 분량은 **본문 산문 문자**로 맞춥니다. Tier A는 15,000~21,000, B는 13,000~18,000, C는 12,000~17,000입니다. ⚠️ **이 숫자는 편의용 사본입니다.** 정본은 `scripts/lint-lesson.sh`와 `docs/course-plan.md` §4 두 곳이고, 어긋나면 **lint가 이깁니다.** 밴드 숫자를 여러 문서에 복사해둔 것이 3중 불일치를 두 번 재발시켰습니다(§9.11, §9.19).
> **이 밴드는 잠정입니다.** 문서가 3층으로 갈리면서 산문 구성이 바뀌므로 W1-M1 시범 결과로 1회 교정합니다. 초안은 중앙을 노리세요. 윤문이 산문을 2~5% 늘립니다. **A4 장수는 지표가 아닙니다.**
