...
로봇을 움직이는 프로그램은 하나가 아닙니다. 이유를 알기 위해서는 로봇 이전에 기계를 움직여 온 방식을 먼저 봐야 합니다. 지금 로봇 스택이 그것을 이어 받았기 때문입니다. 이 절은 그 방식을 처음부터 설명한 뒤, 언어 모델이 지나온 순서가 로봇에서 어떻게 반복되는지를 봅니다.

### 1.1 하나로 만들면 왜 안 되는가

기계를 원하는 대로 움직이는 기본 방법은 **피드백 루프**입니다. 지금 상태를 측정하고, 설정 값과 비교하고, 오차를 줄이는 명령 생성을 쉬지 않고 반복합니다. 팔꿈치의 목표가 30도인데 재보니 28도라면 2도만큼 밀고 1밀리초 뒤에 28.5도면 1.5도만큼 밉니다. 1초에 천 번 돌면 관절은 멈춘 것처럼 30도를 지킵니다. 30도처럼 루프가 따라가야 할 목표값이 **setpoint**이고 1초에 몇 바퀴 도느냐가 그 루프의 속도입니다.

그 30도는 어디서 정할까요. 컵을 집으려면 영상에서 컵을 찾고 팔이 지날 길을 짜야 하는데, 이 판단은 훨씬 무거워 1초에 천 번 할 수 없습니다. 그래서 판단하는 쪽과 따라가는 쪽을 다른 루프로 떼어놓습니다. 느린 바깥 루프가 목표값을 정해 빠른 안쪽 루프에 넘기고 안쪽은 그 값만 보고 달리는 두 겹 구조가 **캐스케이드 제어**입니다. 안쪽을 바깥의 다섯에서 열 배로 돌리라는 경험칙이 붙어 다닙니다. 그래야 바깥이 목표값을 바꾼 순간 안쪽이 이미 따라잡은 상태가 되기 때문입니다. 왜 그 배수인지는 §3.3에서 확인합니다.

로봇 스택도 이 모양이고 **달라진 것은 바깥 루프의 내용물뿐입니다.** 물리 법칙을 적은 수식이 있던 자리에 지금은 수십억 파라미터 신경망이 앉아 있습니다. 그 수식 자리의 대표 격인 **MPC(Model Predictive Control, 모델 예측 제어)**는 앞으로 몇 스텝의 명령을 통째로 계산해 맨 앞 하나만 실행하고 다음 순간에 다시 풉니다. 한 번에 내다보는 스텝 수가 **예측 지평(Prediction Horizon)**입니다. 정확한 예측을 하기 위해 움직임을 수식으로 표현할 수 있어야 합니다. 하지만 다음 이유 때문에 잘 작동하지 못했습니다.
...
Physical AI는 이 내가지를 수식 모델링 대산, 학습으로 대체하고 내부 루프(고속)는 그대로 사용합니다. 신경망 하나가 전체를 처리하는 구조가 아니라 학습과 고전 제어가 층으로 섞인 구조이고 이 모듈이 그 설계도입니다.
...
> 그런데 층을 만든 것은 기술 수준이 아니라 연산 속도 때문입니다. 1초에 천 번 돌 일과 한 번에 0.1초 걸리는 일을 같은 루프에 넣을 수는 없습니다. 모델이 커지면 경계선이 옮겨질 뿐 경계는 남습니다.
...
이 변환이 왜 중요한지가 요점입니다. 모델이 다음에 올 것을 예측하려면 후보가 유한해야 확률을 계산할 수 있습니다. 지금까지 나온 기호를 조건으로 다음 기호의 확률분포를 예측하기를 반복하는 방식을 **자기회귀**라고 하고 그때 쓰는 유한한 후보 목록을 **코드북**이라고 부릅니다.
...
이 모듈은 기술 하나를 가르치는 모듈이 아닙니다. 앞으로 4주 동안 읽을 논문과 회사 도구를 어디에 놓고 봐야 하는지 정하는 지도, 곧 좌표계를 먼저 그리는 모듈입니다. 예를 들어 Diffusion Policy와 SONIC 중 무엇이 상위 개념인지 물으면 답은 "둘은 상하 관계가 아니라 서로 다른 층에 산다"인데, 5계층 그림이 머리에 있어야 이 답이 바로 보입니다.

이 모듈이 세워 두는 기준은 다섯 가지이고 각각 다음 절에서 만듭니다.

- **5계층과 각 층의 주파수** (§3): 로봇 소프트웨어를 하드웨어(L1)부터 인지(L5)까지 다섯 층으로 나누고 각 층이 1초에 몇 번 도는지 적습니다.
- **지연 예산 부등식** (§4): 상위 모델이 명령 하나를 만드는 시간이 아래층이 명령 없이 버틸 수 있는 시간보다 짧아야 한다는 조건입니다. 여기서 한 번에 미리 뽑아야 하는 액션 스텝 수가 계산됩니다.
- **승부처가 L3라는 논증** (§3.4): 상위 모델과 하위 제어기를 잇는 자리가 가장 어렵고 중요하다는 주장과 그 근거입니다.
- **데이터 피라미드** (§5): 로봇 학습 데이터를 양은 많지만 액션 라벨이 없는 것부터 정확하지만 적은 것까지 층으로 쌓은 그림입니다.
- **회사 스택 배치도** (§7): 회사가 쓰는 도구를 5계층 위에 얹은 그림입니다.

이 다섯이 뒤의 어느 모듈에서 다시 쓰이는지는 [deep-dive.md](deep-dive.md) §8.2의 표에 정리했습니다. 마지막 항목은 성격이 다릅니다. 회사 스택 배치도는 내부 구현을 확인하고 그린 것이 아니라 공개 자료로 추측한 **가설**입니다. 그 가설을 팀에게 확인받고 고치는 것이 W4-M5 캡스톤이고, 4주 과정 전체가 이 그림을 검증된 그림으로 바꾸는 과정입니다.
...
> 📌 **여기까지 정리**
> - 로봇 스택은 캐스케이드 루프 제어방식을 사용해왔고, Physical AI에서 바깥 루프가 신경망으로 바뀌었습니다
> - 언어 모델은 백본, 어댑터, 토크나이저, 데이터 등으로 구성되어 있고, 로봇에서도 비슷합니다
...
§1에서 제어 구조가 어떤 이유로 그렇게 구성되었는지 설명했지만, 왜 지금 화두가 됐는지는 아직 설명하지 않았습니다. 이 절은 Physical AI가 무엇을 가리키는지 정의하고, 언어 모델과 무엇이 다른지 세가지로 구분하고, 하필 지금인 이유를 짚습니다.
...
**L4는 상위 지능이고 1초에 1회에서 10회 돕니다.** 카메라 이미지와 언어 지시, 그리고 관절각과 각속도와 몸통 기울기처럼 로봇이 자기 몸에서 읽는 값을 받습니다. 이 내부 감각이 **proprioception(고유감각)**입니다. 내놓는 것은 앞으로 여러 스텝의 동작을 묶은 행렬이고 이 묶음이 **액션 청크**입니다. 16스텝을 예측하고 구동 관절이 29개면 16행 29열 행렬인데, 독립적으로 움직일 수 있는 축의 개수인 **DoF(Degrees of Freedom, 자유도)**가 그 열 수를 정합니다. 이런 계열이 **VLA(Vision-Language-Action, 시각-언어-행동)** 모델입니다.
...
층을 하나씩 따라갔으니 전부를 한 화면에 펼쳐보겠습니다. 아래로 갈수록 빨라지고 단순해집니다. 그림 오른쪽 열에는 우리 회사에서 쓰는 기술이 적혀있는데, 지금은 이름만 눈에 익혀 두세요. 도구가 각각 무엇인지는 §7 「회사 스택 연결」에서, 월드모델은 §5.2 「라벨 없는 상단을 끌어내리는 세 갈래」에서 풉니다.
