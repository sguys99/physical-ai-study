...
PYCELL
  ...
  #
  # ⚠️ **이 밴드는 잠정입니다 (2026-09-01 상향).** 2차 규약이 절 시작 3열 용어표를
  #    폐지하고(§3.8) ASCII 블록도를 그림으로 옮기고(§3.2) 표를 산문으로 푸는데,
  #    위 prose_metrics()의 산문 정의가 그 전환을 **순수 증가**로 셉니다.
  #    보호 구간이 W1-M1은 51.2% → 38%, W1-M2는 52.4% → 32%로 떨어졌고,
  #    W1-M2의 산문 증가 +13,878 중 약 6,600자가 전환분입니다.
  #    표본이 둘뿐이라 이 숫자로 FAIL을 내면 남은 모듈 집필이 미확정 숫자에
  #    끌려갑니다. **밴드를 지키려고 설명을 깎는 것이 §9.19가 진단한 2차 불만의
  #    원인**이었습니다. 7개가 전부 재작성된 뒤 실측으로 확정하고 FAIL로 복귀합니다.
  case "$tier" in
    # 2026-09-01 잠정 상향. 2026-08-10에 세 밴드를 각 +3,000 했고(고정 바닥을
    # 6,350자로 추정했으나 실측 셋이 8,348~8,711이었다, §9.15), 2026-08-30에
    # Tier A 상한만 21,000 → 23,000이었다(§9.20). 이번은 2차 규약의 구조물 산문화를
    # 반영한 것이라 A와 B를 함께 올린다. 근거는 docs/course-plan.md §4.
    A) lo=21000; hi=32000 ;;
    B) lo=18000; hi=26000 ;;
    ...
    #    B와 같은 밴드를 쓰고 아래에서 WARN을 냅니다.
    C) lo=18000; hi=26000 ;;
    *) lo=0; hi=0 ;;
  esac
  # 아래 메시지가 「Tier C 밴드(18000~26000)」처럼 읽히면 안 되므로 표시용 티어를 따로 둡니다.
  local band_tier="$tier"
  if [[ "$tier" == "C" ]]; then
    band_tier="B"
    c_warn "Tier C는 2026-09-01에 폐지됐습니다. frontmatter를 tier: B로 고치세요 — §4: C의 근거 「복습이다」와 「제어 배경이면 즉시 이해된다」를 §3.9(2026-08-25)가 무효화했습니다. 아래 분량 판정은 B 밴드로 냈습니다"
  fi
  ...
  if [[ "$hi" -eq 0 ]]; then
    ...
    if   [[ "$prose" -lt "$lo" ]]; then c_warn "본문 산문 ${prose}자 — Tier ${band_tier} 밴드(${lo}~${hi}) 미달, ${reg_note}"
    elif [[ "$prose" -gt "$hi" ]]; then
      # 2026-09-01: 2차 규약 재작성본의 밴드 **초과**는 FAIL이 아니라 WARN입니다.
      # 판정 신호는 같은 디렉토리의 eli5.md 존재입니다. 2차 규약이 그것을 필수
      # 산출물로 규정했으므로(§2, §2.1) 있으면 2차 규약본, 없으면 1차 규약본입니다.
      # 위 「잠정입니다」 주석이 이유이고, 7개가 전부 재작성되면 FAIL로 복귀합니다.
      if [[ -f "$(dirname "$f")/eli5.md" ]]; then
        c_warn "본문 산문 ${prose}자 — Tier ${band_tier} 밴드(${lo}~${hi}) 초과, ${reg_note}. eli5.md가 있어 2차 규약 재작성본으로 판정했고 밴드가 잠정이라 WARN입니다(§4 2026-09-01 박스). 자르지 마세요 — 밴드를 지키려고 설명을 깎는 것이 2차 불만의 원인이었습니다. 덜어낼 것이 있으면 deep-dive.md로 이관하세요"
      else
        c_fail "본문 산문 ${prose}자 — Tier ${band_tier} 밴드(${lo}~${hi}) 초과, ${reg_note}. 자르지 말고 deep-dive.md로 이관"
      fi
    else c_pass "본문 산문 ${prose}자 (Tier ${band_tier} 밴드 ${lo}~${hi}), ${reg_note}"
    fi
  fi
  ...
  c_info "총 ${total}자 · 보호 구간 ${protect}% (course-plan §1: lesson.md는 비율과 무관하게 윤문 산문 라인 판정)"
