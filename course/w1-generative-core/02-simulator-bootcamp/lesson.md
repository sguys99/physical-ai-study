...
| **"`timestep`을 줄이면 항상 정확해진다"** | MJX 모델은 `timestep`을 오히려 0.002 → **0.004로 키웠습니다**(대신 `iterations` 100 → 5, `kp` 500 → 75, 발목·손목은 20). 이유는 셋입니다. ① 정확도는 `timestep` 단독이 아니라 **`timestep` × solver 반복 × 액추에이터 강성**의 합작입니다. `timestep`만 줄이고 `iterations`가 부족하면 접촉이 여전히 틀립니다. ② 처리량이 학습 시간을 지배합니다 — 스텝당 비용 × 스텝 수. ③ 근본적으로, **접촉 모델 자체가 실기와 다르면 `timestep`을 줄여도 "틀린 답에 더 정밀하게 수렴"할 뿐**입니다. 이산화 오차와 모델링 오차는 다른 문제입니다. |
