...
| 하드웨어 | **로컬 RTX 3080 12GB (WSL2 CUDA 가능) + 필요시 클라우드 GPU** *(v1.2에서 정정 — 이전 "로컬 GPU 없음"은 사실과 달랐음)* | 실습 주 경로(MuJoCo + mujoco_playground, 전부 pip 설치·headless 친화)는 그대로 유지. **W1~W2는 로컬로 완결 가능**하므로 클라우드는 W3 대규모 병렬 학습에만 선택적으로. Isaac Lab의 하드웨어 장벽(RT 코어)은 해소됐으나 WSL2 환경 검증이 없어 **[P1] 유지**. 부록 A를 로컬 우선 → 클라우드 보조 순으로 재작성 |
...
> ⚠️ **아래 「레버리지」 표는 학습 속도에 대한 기대치이지 교육자료의 난이도 전제가 아닙니다** (2026-08-25 신설, 근거는 `docs/course-plan.md` §9.19)
>
> 이 표를 자료 집필의 난이도 전제로 쓴 것이 학습자 불만의 원인이었습니다. "Control 10년이니 W3는 2배 빠르게"가 "제어 개념은 설명하지 않아도 된다"로 번역됐고, 자료가 캐스케이드 제어와 MPC와 대역폭을 앵커로 삼아 설명을 생략했습니다. 학습자는 두 차례에 걸쳐 "빗대어 설명하는데 이해가 안 된다"고 지적했습니다.
>
> **교육자료의 학습자 전제는 `docs/course-plan.md` §3.9가 정합니다.** 안다고 가정하는 것은 ML/DL 기본과 LLM 기본과 Agent·RAG 실무 셋뿐이고, 제어공학과 생성모델 내부 기제와 로봇 전 영역은 **전부 기초부터** 씁니다. 이 표는 일정 배분의 근거로만 씁니다.

| 보유 역량 | Physical AI에서의 레버리지 (일정 배분 근거) |
|---|---|
| Control 10년 | W3(WBC/RL/sim2real)에 일정을 덜 배정해도 됨. 단 **자료는 기초부터 씁니다** |
| ML/DL | W1 생성모델 파트에 일정을 덜 배정해도 됨. 단 **생성모델 내부 기제(BPE, autoregressive, codebook, ELBO)는 갭이므로 정의부터** |
| LLM/Agent/RAG | VLA의 VLM 백본과 시스템1/2 아키텍처의 **큰 그림**이 빨리 잡힘. 토크나이저 내부는 갭 |
...
[학습자 전제] 주니어에게 처음부터 설명한다고 생각하세요.
  안다고 가정하는 것은 셋뿐입니다: ML/DL 기본(학습·손실·신경망·과적합), LLM 기본(프롬프트·
  토큰이라는 말·파인튜닝), Agent와 RAG 응용 실무.
...
산출물은 3층입니다 (docs/course-plan.md §2.1):
  eli5.md    그림 위주 입문. 산문 3,000~5,000자. 수식 0, arXiv 인용 0, 영문 약어 최소
  lesson.md  본문. 아래 구조와 분량
  deep-dive.md  수식 전체 유도, 계보 세부, 출처 검증 메모 (덜어낼 것이 있을 때만)

lesson.md 구조 (순서 고정, 상세는 docs/course-plan.md §3.7):
  한 줄 요약 → §0 이 모듈 지도(오늘의 질문 3개 + 목차 표 + Mermaid 지도)
  → §1 배경(왜 배우나 + 선수 지식 체크)
  → §2~N 단계별 상세. 절마다 [도입 산문 3~5문장] → 본문 → [되짚기 표] → [📌 정리 3줄]
  → 회사 스택 연결 ★ → 흔한 오해 → 한 장 정리 → 셀프 체크 퀴즈 10문항 → 출처

요구사항:
1. **비유는 보조입니다.** 본문은 비유 없이 완결되어야 합니다. 비유는 본문 뒤에
   `<details><summary>💡 제어를 아신다면 (몰라도 됩니다)</summary>` 박스로 격리하고,
   박스 안에서 앵커 개념을 **한 문단으로 재설명**하세요. 절당 최대 1개.
   용어표에 `제어·LLM 비유` 열을 두지 마세요.
2. **대절 제목 다음 첫 요소는 반드시 도입 산문**입니다. 표도 그림도 코드도 아닙니다.
   앞 절에서 무엇을 봤는지, 그래서 무엇이 안 풀렸는지, 이 절이 무엇에 답할지 세 문장.
3. 절 안의 순서는 **개념 → 그림 → 예시와 숫자 → 수식과 손계산 → 비유 박스**.
4. **수식에는 숫자를 대입한 손계산이 붙습니다.** 결과만 제시하고 계산을 실습 코드로
   외주하지 마세요. 모든 기호는 그 자리에서 정의하고, 기호가 셋을 넘으면 기호표를 답니다.
5. **그림을 적극 쓰고 「읽는 법」 캡션을 답니다.** 개념 그림은 SVG로 만들어 img/에 두고
   ../../../img/로 참조합니다. Mermaid는 구조·흐름·계보에. ASCII 블록도는 한 블록 20줄 상한.
6. **용어**: 첫 등장 자리에서 두세 문장으로 풀어 쓰고 굵게 표시. 정의보다 먼저 쓰지 마세요.
   한 줄 정의는 절 끝 되짚기 표에만. 약어는 최초 1회 전개하고 첫 세 번은 한국어 병기
   (`MPC(Model Predictive Control, 모델 예측 제어)`). 다이어그램 라벨에서 용어 첫 소개 금지.
7. **표는 축이 2개 이상인 진짜 비교만.** 나열·순차·근거는 불릿, 논증은 산문.
   표 10행·셀 120자 상한. 표 블록 수 ≤ 불릿 블록 수.
8. **설명 회피 문장 금지.** "제어 배경이면 설명이 필요 없을 겁니다", "당신은 이미 알고 있다",
   "익숙하실 겁니다" 류. 생략할 거면 대신 링크를 주세요.
9. 흔한 오해 3가지 이상과 교정. 오해가 생기기 쉬운 자리에는 그 자리에
   `> ⚠️ **여기서 많이 헷갈립니다**` 박스를 답니다.
10. 셀프 체크 퀴즈 10문항을 3단 난이도로 (기억 확인 4 / 적용 4 / 종합 2).
    답은 `<details>`로 접어서 뒤에. 본문·📌 박스·표·퀴즈를 전부 합니다체로 통일.

분량: lesson.md 본문 산문 문자는 **하한만** 있습니다. Tier A는 21,000 이상, Tier B는 18,000 이상.
Tier C는 2026-09-01에 폐지됐습니다(W1-M3 / W1-M4 / W4-M1이 B로 이동).
상한은 문자가 아니라 **초독 시간**입니다. `est_reading_min`이 180분을 넘으면 안 됩니다(150분 초과는 경고).
`est_reading_min`은 표·그림·수식·퀴즈를 포함한 초독 시간이고 §0의 「소요」와 정합해야 합니다.
넘치는 유도·계보 세부는 `deep-dive.md`로 이관하고 본문에서 링크.
**밴드의 정본은 `scripts/lint-lesson.sh`이고 규약 본문은 docs/course-plan.md §4입니다** — 여기 숫자를 복사하지 말고 그 두 곳을 보세요.

웹 검색으로 최신 정보를 보강하고 출처를 남겨주세요.
```
...
- **시뮬 경험 전무가 최대 리스크** → 대응: 주력 실습 스택을 전부 "pip 설치 + headless"로 통일(MuJoCo, mujoco_playground, LeRobot). Isaac Lab은 주 경로에서 제외하고 [P1] 선택 과제로. 단, **회사 학습 인프라가 Isaac 기반일 가능성이 높으니 첫 주에 팀 표준 학습 환경(시뮬레이터·클러스터·도커 이미지)을 반드시 확인**하고, 있으면 그것을 그대로 물려받는 것이 최선.
  *(v1.2 정정: 이전 판은 "클라우드 전용"을 리스크의 한 축으로 봤으나 로컬 RTX 3080 12GB가 확인되어 이 축은 크게 완화됐습니다. 부록 A.0 참조.)*
- **로컬 환경의 리스크**: WSL2 특유의 문제(EGL 정리 단계 traceback — 무해, `libOSMesa` 부재), VRAM 12GB 상한(W3 병렬 환경 개수 제약), 장시간 학습 중 호스트 사용 불가. 대응은 W1-M2 랩의 에러 표와 `--smoke` 우선 원칙.
- **클라우드로 넘어갈 때의 리스크**: 인스턴스 비용 폭주(스팟 활용, 학습 없는 시간 정지 습관), 체크포인트·데이터 유실(퍼시스턴트 볼륨 필수), 렌더링 이슈(`MUJOCO_GL=egl`, 뷰어 대신 mp4 저장). 상세는 부록 A.1.
...
## 부록 A. 실습 환경 가이드 — 로컬 우선, 클라우드 보조

### A.0 확인된 로컬 환경 (2026-08-02 실측)

| 항목 | 값 |
|---|---|
| GPU | **NVIDIA RTX 3080 12GB** (Ampere, compute capability 8.6) · WSL2에서 `nvidia-smi`·CUDA 정상 |
| 호스트 | RAM 62GB · 20 코어 · WSL2 (Ubuntu) |
| 검증됨 | `MUJOCO_GL=egl` 오프스크린 렌더링 · `mujoco 3.11.0` · `playground 0.2.0` · G1 MJCF 로드·구동 |
...
> **Isaac Lab 관련 정정**: RTX 3080은 RT 코어가 있어 **Isaac Sim의 하드웨어 요구는 충족합니다.** 이전 판이 Isaac Lab을 강등한 이유 중 "클라우드 GPU에 RT 코어가 없다"는 근거는 사라졌습니다. 다만 **WSL2에서의 Isaac Sim 구동은 검증하지 않았고**(셋업 난도가 별개로 높음) 4주 일정에 넣을 여유가 없으므로 **[P1] 유지**입니다. 팀 표준이 Isaac이라면(→ 팀 질문 M2-1) 그때 재평가합니다.

### A.1 클라우드가 필요해지는 경우

로컬이 막히거나 느릴 때만 씁니다. 판단 기준은 **"학습 1회가 하룻밤을 넘기는가"** 입니다.

- **인스턴스 선택**: W3(playground RL)를 클라우드로 옮길 경우 A10G/L4/L40S급. A100/H100은 이 플랜에서 불필요. Isaac Lab을 [P1]으로 시도할 경우 RT 코어가 있는 GPU(L4/L40S/A10G 계열) — A100/V100은 Isaac Sim 렌더링을 지원하지 않음.
...
*v1.5 (2026-09-03). **§11.1 템플릿의 분량 규정을 갱신했습니다.** 적혀 있던 `Tier A 15,000~21,000 / B 13,000~18,000 / C 12,000~17,000`이 두 단계 낡았고 2026-09-01에 폐지된 Tier C까지 명시하고 있었습니다. 새 규정은 **산문 문자에 하한만 두고**(A 21,000 이상, B 18,000 이상) **상한을 초독 시간으로 옮긴 것**입니다(`est_reading_min` 180분 초과 금지). 근거는 표본 7개 실측이고, 상한의 바깥 근거는 이 문서 §2의 주 30시간 예산입니다. **커리큘럼 내용(모듈 구성, 논문 목록, 일정)은 바꾸지 않았습니다.** 상세 규약과 판정 기준은 `docs/course-plan.md` §4이고 집행은 `scripts/lint-lesson.sh`입니다.*

*v1.4 (2026-08-25). **§3 갭 분석과 §11.1 템플릿의 난이도 전제를 뒤집었습니다.** W1 학습자가 §9.11 개정 후에도 같은 불만("설명이 친절하지 않고 연결성이 부족하다, 빗대어 설명하는데 이해가 안 된다, 너무 어렵다")을 다시 제기했습니다. 재집필본 W1-M1을 실측하니 비유 지점 50개 중 앵커를 처음부터 설명한 것이 3개뿐이었고, H2 일곱 개가 전부 도입 산문 0자였으며, 논거의 뼈대인 `latent`와 `다중 모드`와 `동특성 간섭`과 `공백률`이 문서 전체에서 미정의였습니다. 워크드 예제는 0개, 이미지 파일도 0개였습니다. 원인은 **§3 갭 분석의 「Control 10년 레버리지」를 자료의 난이도 전제로 쓴 것**입니다. 학습자 전제를 「ML·LLM·Agent 기본만 안다」로 단일화하고, 제어와 생성모델 내부와 로봇을 전부 기초부터 쓰도록 §11.1을 다시 썼습니다. 비유를 접이식 박스로 강등하고, 산출물을 `eli5.md` + `lesson.md` + `deep-dive.md` 3층으로 나눴습니다. **커리큘럼 내용(모듈 구성, 논문 목록, 일정)은 바꾸지 않았습니다.** 상세 규약과 근거는 `docs/course-plan.md` §2.1 · §3.7~3.9 · §3.11 · §4 · §9.19이고 실행 계획은 `docs/course-author-update-plan.md`입니다.*

*v1.3 — 2026-08-09. **§11.1 학습자료 생성 템플릿 개정.** W1 학습자 피드백("체계가 없고 두서없다 / 너무 어렵다 / 용어 설명이 부족하다 / 줄글이라 직관적이지 않다")을 실측 진단한 결과, 규칙 부재가 아니라 **있는 규칙이 잘못된 것을 최적화하고 있었음**이 확인됐습니다 — 목차가 저장소에 하나도 없었고, "첫 등장 시 정의하라"는 규칙이 없었으며, `비교는 무조건 표`가 과적용돼 논증까지 표 셀에 들어가 있었습니다. §11.1에 **섹션 순서 고정 · 용어 도입 규칙 · 표 사용 조건 · 이해 사다리**를 추가하고 **분량 단위를 A4 장수에서 산문 문자로 교체**(A4 환산이 문서마다 3중 불일치)했습니다. 상세 규약과 근거는 `docs/course-plan.md` §3.7~3.9 · §4 · §9.11.*

*v1.2 — 2026-08-02. **하드웨어 전제 정정.** W1-M2 집필 중 실습 환경에 **RTX 3080 12GB(WSL2 CUDA 가능)** 가 있음이 확인되어, v1.1의 "로컬 GPU 없음 / 클라우드 전용" 전제를 §1·§12·부록 A에서 바로잡음. 실습 주 경로(MuJoCo + mujoco_playground)는 변경 없이 유지하되 **W1~W2는 로컬 완결, 클라우드는 W3 대규모 병렬 학습에만 선택적**으로 재배치. 부록 A를 "로컬 우선 → 클라우드 보조" 구조로 재작성하고 A.0(로컬 실측 사양·주차별 처리 가능 여부)을 신설. Isaac Lab은 하드웨어 장벽이 해소됐으나 WSL2 검증 부재로 [P1] 유지.*