...
> **밴드에 상한이 없습니다.** 2026-09-03에 확정됐습니다. 산문은 하한만 보고 상한은 `est_reading_min`으로 옮겨졌습니다(180분 초과 FAIL, 150분 초과 WARN). 숫자는 `scripts/lint-lesson.sh`와 `docs/course-plan.md` §4 두 곳이 정본입니다. 초안은 중앙을 노리세요. 윤문이 산문을 2~5% 늘립니다. **A4 장수는 지표가 아닙니다.** **Tier C는 2026-09-01에 폐지됐고 `tier:`에 넣을 값은 A와 B뿐입니다.**
...
<!-- ②-2 그림. Mermaid는 흐름과 계보와 타이밍에, ASCII는 차원 표기가 필요한 블록도에,
     SVG는 공간 배치가 의미를 나르는 개념 그림에 (직접 작성해 img/에 저장).
...
- **Mermaid는 흐름과 계보와 타이밍에**, **ASCII는 차원 표기가 필요한 블록도에**, **SVG는 공간 배치가 의미를 나르는 개념 설명 그림에** 씁니다. 계층 스택과 타이밍과 피라미드가 SVG의 대표 사례이고, SVG는 직접 작성해 `img/{module-id}-{slug}.svg`에 두고 본문에서 `../../../img/`로 참조합니다
