...
| §5 | 분리형과 통합형의 일곱 축 비교 | lesson §7.4 「분리형인가 통합형인가」 |
...
본문 §7.4 「분리형인가 통합형인가」가 「업계 합의는 없습니다」로 줄인 대립을 축별로 폅니다. 2026-07-31 공개된 Gemini Robotics 2가 회사 구도와 직접 비교 대상인 이유는 방향이 정반대이기 때문입니다.
...
| 6 | 본문 §3.2 「입출력과 주파수를 한 장에」의 기호표 「회사 실제값」 열과 §7.1 「배치도」 | 회사 내부 구현 전체 | 전부 「팀 확인 필요」로 남김 |
...
- **L2.** 관측 이력 스택 `[T_hist, D_obs]`을 쌓는 이유는 속도와 가속도 정보가 단일 프레임에 없기 때문입니다. 신경망 순전파 한 번에 균형과 접촉과 모션 트래킹이 전부 걸립니다
...
**10. 회사 도구 다섯의 배치와 모르는 것 셋.** 배치는 L5가 DualMap(W4-M3), L4가 VLA와 월드모델(W2-M3과 M4, W4-M1과 M2), L3이 FSQ 기반 계층 모델(W1-M5와 W2-M5), L2가 GEAR-SONIC(W3-M4)과 HOMIE(W3-M3), L1이 Unitree G1(W1-M2와 W3-M5)입니다.

모르는 것 셋은 이렇습니다. 첫째, L4에 무엇이 앉는지가 미정입니다. 자체 VLA인지 공개 모델 파인튜닝인지 아직 정해지지 않았는지 알 수 없습니다. 둘째, L3과 L2의 접점이 어떤 형태인지 모릅니다. FSQ 토큰이 목표 자세인지 모션 레퍼런스인지 latent인지에 따라 설계가 완전히 달라집니다. 셋째, DualMap이 L4로 무엇을 넘기는지 모릅니다. 본문 §3.1 「명령 하나가 맨 위에서 맨 아래까지 내려간다」의 좌표 표기는 추정입니다.

배치도 전체가 **가설**이라는 점이 이 문항의 핵심입니다. 셋을 질문으로 뽑아 팀에 확인하고 그림을 고치는 것까지가 캡스톤 과제입니다.
