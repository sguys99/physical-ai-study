...
| 트랙 | 완료 | 비고 |
|---|---|---|
| W1 모듈 (생성모델 코어) | 0 / 5 | **W1-M1 · W1-M2 · W1-M3 · W1-M5 ★ 집필 4종 완료**, 학습자 실행검증만 남음 · 남은 것은 **W1-M4 하나** |
| W2 모듈 (정책·VLA) | 0 / 5 | — |
| W3 모듈 (WBC·RL) | 0 / 5 | — |
| W4 모듈 (월드모델·내비·통합) | 0 / 5 | — |
| 논문노트 3-pass | 1 / 6 | **FSQ 완료** · SONIC · HOMIE · pi0 · DP · DualMap |
| 리포 투어 | 0 / 4 | OpenHomie · SONIC · unitree_rl_gym · DualMap |
| 용어집 (목표 100개) | 0 / 4주차 | 주차 종료 시 25개씩 · 현재 **50개**(기본 10 + W1-M1 10 + W1-M2 10 + W1-M3 10 + W1-M5 10) — W1 목표 25개 초과 달성 |
| 팀 질문 | — | 23건 적립 (P0 5 + W1-M1 6 + W1-M2 5 + W1-M3 6 + W1-M5 6, 중복 제외) |
| 캡스톤 | 0 / 1 | 스택 해설 문서 + 발표 |

> 🚧 **데드락 방지선 상태 — 해소.** W1 모듈 **4/5** 확보(M1·M2·M3·M5). §7의 "주차 시작 시 최소 3개" 기준 충족. **끝까지 지킬 것(§8) 중 FSQ 구현(W1-M5)도 완료.**
> 남은 W1: W1-M4(Flow Matching, Tier C) 하나. W1-M3 랩이 §5.3의 "NFE 1~2는 샘플러 교체로 안 뚫린다"를 실측으로 남겼으므로 W1-M4는 그 근거를 이어받아 집필합니다.
...
| 6 | **보존 검증** | 스킬 | 윤문 전후 diff 대조. 보존 대상이 바뀌었으면 롤백. 변경률 판정은 **티어별 지표**를 씁니다(아래 박스) |
...
> 📏 **변경률 판정 지표 — 티어별로 다릅니다** (W1-M2 경험 반영, §9.1)
>
> | Tier | 판정 지표 | 정상 밴드 |
> |---|---|---|
> | **A** | **산문 라인 변경률**(코드펜스·표 행 제외) + **잔존 burden 감소율** | 라인 20~50% · burden 80%↓ 이상 |
> | B · C | 전체 문자 변경률 | 5~30% |
>
> Tier A 문서는 문자의 40% 이상이 코드·표·수식(전량 보존 대상)이라 **전체 문자 기준 변경률이 구조적으로 낮게 나옵니다.** W1-M2는 1.48%였지만 S1 5/5·S2 41/43 해소에 산문 라인 34.2% 변경으로, 저윤문이 아니었습니다. 문자 변경률만 보고 재윤문을 지시하면 **과윤문을 유발**합니다.
>
