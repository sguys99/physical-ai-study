...
[`../lesson.md`](../lesson.md)의 §3.3(5계층 스택) · §3.4(대역폭 분리) · §5.1(데이터 피라미드) · §4(지연 예산) · §6.3(플레이어 계보)를
...
| 3 | `03_player_landscape.py` | `players.csv`(lesson §6.3 표)를 읽어 조직×베팅 계층 매트릭스 + 발표 타임라인 렌더 | `03_player_landscape.png` |
...
lesson §6.3 표를 CSV로 옮긴 것입니다. 이 분야는 몇 주 단위로 새 모델이 나오므로,
...
- **1차 출처로 확인되지 않은 수치는 넣지 마세요.** lesson §6.3이 의도적으로 배제한 수치
...
- **`03`의 매트릭스** — **L3 열이 비어 있다는 것**이 이 그림의 핵심입니다. 공개 논문 수준에서 액션 인터페이스를
  독립 계층으로 내세운 조직이 드물고, lesson §3.5의 "승부처는 L3"와 맞물립니다.
