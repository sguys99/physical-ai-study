...
- **변경률 판정은 티어별 지표를 씁니다** (SSOT: [`docs/course-plan.md`](../../../docs/course-plan.md) §9.1)
...
- ⚠️ **전체 문자 변경률이 낮다는 이유만으로 재윤문을 지시하지 마세요.** Tier A 문서는 문자의 40% 이상이 코드·표·수식(전량 보존 대상)이라 전체 문자율이 구조적으로 낮게 나옵니다. W1-M2는 전체 1.48%였지만 S1 5/5·S2 41/43 해소에 산문 라인 34.2% 변경으로, 저윤문이 아니었습니다. 이 오판으로 재윤문을 돌리면 **과윤문을 유발합니다**(§9.1).
