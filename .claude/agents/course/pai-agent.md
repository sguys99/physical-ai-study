...
- **보유**: Control engineering 10년(동역학·상태공간·최적제어·MPC) + 데이터 사이언티스트 / ML 엔지니어 / 에이전트 엔지니어(LLM·Agent·RAG **응용** 실무)
- **미보유**: 로봇 시뮬레이터 실무(**경험 전무**), 로봇 RL 학습 실무, 모방학습/VLA 파이프라인, sim2real·로봇 미들웨어, 3D 비전/SLAM, **생성모델·LLM 내부 기제**
...
## 회사 스택 연결 ★      ★필수
## 흔한 오해 3가지 이상   ← 표가 아니라 소제목 + 산문 (논증은 셀에 안 들어감)
...
**절 안의 순서: 개념 → 직관 → 수식 → 그림.** 형식이 서사보다 먼저 오면 독자는 자기가 무엇을 보는지 모르는 채로 다이어그램을 마주칩니다. (W1-M1 §3이 이 실패 사례 — 68줄짜리 ASCII 블록도가 "계층별로 읽기" 서사보다 앞에 있었습니다.)

### 용어 도입 (SSOT: §3.8) — 지금까지 없던 규칙이니 특히 유의
...
- 목표 독해 **33~38분**. 산문 = 코드펜스·`$$`·표 행·frontmatter 제외 문자
- **넘치면 자르지 말고 `deep-dive.md`로 옮기세요** — 수식 전체 유도 · 계보 연대기 세부 · 트레이드오프 심층 · 1차 출처 검증 메모. 본문에는 결과와 직관을 남기고 `(유도 전체는 [deep-dive.md](deep-dive.md) §2)`로 링크. **덜어내는 것이지 버리는 것이 아닙니다**
...
최소 구성: ① **모듈 지도** ② 계보·지형 ③ 아키텍처 ④ **회사 스택 연결도** ⑤ 비교표.
...
- **표 1개당 10행 상한 · 셀 1개 120자 상한.** 넘으면 축을 쪼개거나 불릿으로
- **표 블록 수 ≤ 불릿 블록 수**
...
0. **규약 린트 — 가장 먼저 돌립니다.** 구조·용어·표 규약은 스크립트가 검사합니다.
   ```bash
