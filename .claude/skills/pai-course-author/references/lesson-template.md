...
> **섹션 순서는 고정입니다.** SSOT는 `docs/course-plan.md` §3.7(구조) · §3.8(용어) · §3.9(이해 사다리) · §3.2(시각자료) · §4(분량).
> 분량은 **본문 산문 문자**로 맞춥니다 — Tier A 12,000~18,000 · B 10,000~15,000 · C 9,000~13,000 (초안은 중앙을 노릴 것 — 윤문이 2~5% 늘립니다). **A4 장수는 지표가 아닙니다.**
> 넘치는 유도·계보 세부는 같은 폴더의 `deep-dive.md`로 빼고 본문에서 링크합니다.
...
<!-- ASCII 블록도 — 텐서 shape·차원이 필요한 곳만. 입출력과 차원을 반드시 명시 -->
```
[입력: x  (B, T, D)]
        │
        ▼
...
<!-- 3가지 이상. 논증이 필요하므로 표가 아니라 소제목 + 산문으로 씁니다 -->
...
- 대절(H2)마다 **용어표(신규 용어가 있으면) + `📌` 정리 박스**. `📌`만 이어 읽어도 줄거리가 잡혀야 합니다
...
**시각자료** (§3.2)
- **5종 이상**: ① 모듈 지도 ② 계보·지형 ③ 아키텍처 ④ 회사 스택 연결 ⑤ 비교표
- **Mermaid는 흐름·계보·타이밍**, **ASCII는 차원 표기가 필요한 블록도**
- **표는 축이 2개 이상인 진짜 비교만.** 나열·순차·근거는 불릿, 논증은 산문 + 소제목
- **표 10행 · 셀 120자 상한. 표 블록 수 ≤ 불릿 블록 수**

**난이도** (§3.9)
- 제어·ML은 기초 생략 가능. 단 **앵커로 쓰는 개념은 그 자리에서 한 줄 재진술**
- **LLM 응용 실무(Agent·RAG)는 있으나 생성모델 내부 기제(BPE·autoregressive·codebook·ELBO)는 갭** — 가정하지 말 것
- 로봇 시뮬·VLA·WBC·SLAM은 기초부터
- **설명 회피 문장 금지** ("제어 배경이면 설명이 필요 없을 겁니다") — 생략할 거면 링크를 줄 것
