...
_V1_METRICS_DIR = os.path.join(_PROJECT_ROOT, ".claude", "skills", "humanize-korean", "references")
    ...
    for s in sents:
        ...
        for josa in ("은", "는", "이", "가", "도"):
            if head.endswith(josa) and len(head) > 1:
                head_stem = head[:-1]
                break
        is_inanimate = head_stem in inanimate_subjects or (
            len(head_stem) >= 2 and head_stem[-1] in _HANJA_SUFFIXES_V2
        )
        if not is_inanimate:
            continue
...
def compute_all_v2(
    ...
    for k, v in v2_metrics.items():
        ...
        if cell.get("_placeholder"):
            warnings.append(k)
        z_scores[k] = _z_simple(float(v), float(cell.get("mean", 0.0)), float(cell.get("stdev", 0.0)))
...
def _main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--baseline", default=None, help="Override v1.6 baseline JSON path")
    parser.add_argument("--baseline-v2", default=None, help="Override v2.0 baseline JSON path")
