...
4. **세로 점선 두 개는 무엇인가?** G1 classic(`0.002`)과 MJX(`0.004`)의 위치입니다. **MJX가 어느 방향으로 이동했는지** 보세요. `timestep`을 줄이는 것이 항상 정답이 아니라는 것이 [lesson §9](../lesson.md)의 두 번째 오해입니다.
...
> 이 관찰이 **HOMIE** ★ 의 설계 근거와 이어집니다 — 하체는 RL로 균형을 학습하고 상체는 텔레옵으로 조종하는 분리 구조 (lesson §8.2, W3-M3).
...
**[lesson §10](../lesson.md)의 5개 중 최소 3개를 옮기세요.** 우선순위가 가장 높은 셋:
...
> 그리고 **회사 클라우드 계정이 있으면 그것을 우선하세요**(마스터플랜 부록 A). 그 경우 사내 승인 절차·태그 정책·예산 알림 설정을 먼저 확인해야 합니다 → lesson §10 질문 5.
