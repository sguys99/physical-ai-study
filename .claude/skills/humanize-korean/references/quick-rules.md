...
**과윤문 가드:** 변경률 30% 초과 = 경고, 50% 초과 = 강제 중단·롤백. (**`register: course`는 strict로 라우팅되므로 이 monolith 룰북 미해당** — 교육자료 밴드는 산문 라인 20~50%로, strict 파이프라인이 적용한다. 하단 §register: course 예외 참조.)

**흔한 어휘 우선 (plain-language 가드):** 번역투·기계적 병렬을 걷어내려다 **일상 기술 문서에서 잘 안 쓰는 문어체·문학체 어휘로 갈아타지 않는다.** 원문보다 더 "글쟁이 티" 나면 과교정이다. 반대 방향도 같은 위반이다. **구어 은유와 영어 관용구 직역**(`칸을 먹다`, `걸려 넘어지는 지점`, `되는 길`)으로 쉬워 보이게 만드는 것 역시 평이한 기술 문서 어휘에서 벗어난 것이고, 이런 어구가 원문에 있으면 윤문이 평이한 서술어로 바꾼다(2026-09-22, W1-M2 eli5 학습자 피드백. 근거는 `docs/course-plan-log.md` §9.22). 관찰된 drift → 평이 치환:
...
구어 은유·관용구 직역 → 평이 (2026-09-22 추가. 학습자가 "영어 직역처럼 어색하다"고 지목한 유형. 집필 단계에서 나오지만 윤문본에 남아 있으면 같은 처리):

| 구어 은유·관용구 직역 (지양) | 평이 (권장) |
|---|---|
| 칸을 먹다 / 자리를 먹다 | 칸을 차지하다 / 자리를 차지하다 |
| 걸려 넘어지는 지점 (stumbling block) | 막히는 곳 / 자주 틀리는 곳 |
| 되는 길 / 안 되는 길 (the way that works) | 실제로 쓰는 방법 / 실패하는 방법 |
| 하루를 날리다 | 하루를 허비하다 |
| 부품이 나가다 | 부품이 고장나다 |
| 손에 잡히는 (tangible) | 구체적인 |
| 계산을 붙들고 있다 | 계산에 시간을 쓰다 |
| 원칙이 하나 붙다 / 하나가 더 붙다 | 원칙이 하나 있다 / 하나 더 있다 |
| 환경을 굴리다 / 영상을 뽑다 (개발자 구어) | 환경을 실행하다 / 영상을 저장하다 |
| 무릎에게 명령을 (무생물에 에게) | 무릎에 명령을 |

> 규칙: 치환어를 고를 때 "이 단어를 일반 기술 블로그·문서에서 쓰나?"를 자문. 아니면 더 흔한 말로. 원문에 없던 비유·수사는 추가 금지(자체검증 6항). 문학체와 구어 은유는 방향만 다르고 판별 기준은 같다.
...
6. **인공 표현 자제**: 원문에 없던 비유·수사·문학적 표현을 윤문 과정에서 임의로 추가하지 않았는가. 위 "흔한 어휘 우선" 대응표의 드리프트 어휘를 새로 집어넣지 않았는가, 둘째 표의 구어 은유·관용구 직역 어구를 새로 집어넣거나 남겨두지 않았는가
