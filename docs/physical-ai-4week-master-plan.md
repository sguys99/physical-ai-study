...
구조 (순서 고정 — 상세는 docs/course-plan.md §3.7):
  한 줄 요약 → §0 이 모듈 지도(오늘의 질문 3개 + 목차 표 + Mermaid 지도)
  → §1 배경(왜 배우나 + 선수 지식 체크)
  → §2~N 단계별 상세 — 절마다 [이 절의 새 용어 표] → 본문 → [📌 여기까지 정리 3줄]
  → 회사 스택 연결 ★ → 흔한 오해 → 한 장 정리 → 셀프 체크 퀴즈 10문항 → 출처

요구사항:
1. 개념 설명: 제어공학·LLM 개념에 빗댄 비유를 적극 사용 (예: FSQ ≈ 액션의 토크나이저).
   단 **비유의 앵커로 쓰는 개념은 그 자리에서 한 줄 재진술**할 것.
2. 절 안의 순서는 **개념 → 직관 → 수식 → 그림**. 형식이 서사보다 먼저 오지 않게.
3. 핵심 수식: 본문에는 결과와 직관만. 전체 유도는 deep-dive.md로 빼고 링크.
4. 아키텍처: 텍스트 다이어그램으로 입출력·차원 명시.
5. 회사 스택 연결: 이 개념이 우리 파이프라인 어디에 쓰이는지.
6. **용어**: 처음 쓰는 용어는 절 시작 용어표에 올리고 첫 등장을 굵게. 약어는 최초 1회 전개
   (`MPC(Model Predictive Control, 모델 예측 제어)`). 한 줄 요약과 §0에는 미정의 용어 금지.
7. **표는 축이 2개 이상인 진짜 비교만.** 나열·순차·근거는 불릿, 논증은 산문.
   표 10행·셀 120자 상한. 표 블록 수 ≤ 불릿 블록 수.
8. **난이도**: 제어·ML은 기초 생략 가능. **LLM 응용 실무는 있으나 생성모델 내부 기제
   (BPE·autoregressive·codebook·ELBO)는 갭**이므로 가정하지 말 것. 로봇 시뮬·VLA·WBC·SLAM은 기초부터.
   "제어 배경이면 설명이 필요 없을 겁니다" 류의 설명 회피 문장 금지.
9. 흔한 오해 3가지 이상과 교정.
10. 셀프 체크 퀴즈 10문항 (답은 `<details>`로 접어서 뒤에).

분량: 본문 산문 문자로 Tier A 12,000~18,000 / B 10,000~15,000 / C 9,000~13,000
(목표 독해 33~38분). 넘치는 유도·계보 세부는 `deep-dive.md`로 이관하고 본문에서 링크.
**A4 장수는 지표로 쓰지 않습니다** — 근거는 docs/course-plan.md §4·§9.11.
