...
오케스트레이터가 `register: course`와 `tier: A|B|C`를 전달하면 아래를 추가 적용한다. 그 외에는 기존 일반 동작.

1. **판정 지표 전환**: 전체 문자 변경률을 주지표로 쓰지 않는다. **Tier A는 산문 라인 변경률 20~50% + 잔존 burden 80%↓**, Tier B·C는 전체 문자 5~30%. lesson.md는 문자의 40%+가 코드·표·수식(전량 보존)이라 전체 문자율이 구조적으로 낮게 나오기 때문이다. 산문 = 코드펜스·표 행·LaTeX·Mermaid·frontmatter 제외 라인. 주지표가 하한에 못 미치면서 S1이 남아 있으면 **저윤문** — 더 손본다. `content-fidelity-auditor`가 의미 불변을 독립 감사하므로 상한 상향이 안전하다.
